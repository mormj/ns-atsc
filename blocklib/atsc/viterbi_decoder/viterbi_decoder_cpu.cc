...
void viterbi_decoder_cpu::reset()
{
    for (int i = 0; i < NCODERS; i++)
        fifo[i].reset();
}

std::vector<float> viterbi_decoder_cpu::decoder_metrics() const
{
...
work_return_code_t viterbi_decoder_cpu::work(std::vector<block_work_input_sptr>& work_input,
